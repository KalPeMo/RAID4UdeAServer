...
if len(sys.argv) > 1:
    ...
    disk = '/dev/md0'  # Default disk if none is specified
...
file_sizes = ['1M', '4M', '16M']  # File sizes
...
with open(csv_file, mode='a', newline='') as file:
    ...
    for file_size in file_sizes:
        for block_size in block_sizes:
            ...
            read_speed = re.findall(r'Read:\s+([0-9]+)\s+B/sec', output)[0]
            write_speed = re.findall(r'Write:\s+([0-9]+)\s+B/sec', output)[0]
            random_read_speed = re.findall(r'Random Read:\s+([0-9]+)\s+B/sec', output)[0]
            random_reread_speed = re.findall(r'Random Reread:\s+([0-9]+)\s+B/sec', output)[0]
            ...
            print(f"Random reread speed: {random_reread_speed}")
            ...
            writer.writerow([timestamp, username, file_size, block_size, read_speed, write_speed, random_read_speed, random_reread_speed, str(elapsed_time)])
