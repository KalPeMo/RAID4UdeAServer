...
def write_to_csv(data):
    with open('iozone_results14-1.csv', 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        if not csvfile.tell():
            writer.writerow(['Benchmark', 'Disk', 'Test', 'Replica', 'file_size_in', 'reg_size_in', 'random_write(KB/s)', 'random_rewrite(KB/s)', 'random_read(KB/s)', 'random_reread'])
...
def run_experiment(file_size, record_size, chunk_size, replicas, experiment_number, total_experiments, disk):
    for replica in range(replicas):
        ...
        write_to_csv(['iozone', disk, experiment_number, replica+1] + last_6_numbers + [difference])
